    ...
    try:
        ...
        if isinstance(num, float):
            if num.is_integer():
                ...
                return False
        return p.number_to_words(num) == s.lower()
    except:
        return False
...
def generate_questions():
        ...
        while len(response_questions) < num_questions and attempts < max_attempts:
            ...
            try:
                ...
                for question_text in question_list:
                    ...
                    result = qg_pipeline(prompt, max_new_tokens=64)
                    question_text = result[0]['generated_text'].strip() if result else None
    ...
    # Replace multiple spaces with single space
                                ...
                                # Fix missing spaces between words if possible (hard but can use spacing models)
                                # Remove strange sequences
                                text = re.sub(r's{2,}', 's', text)
                                # Other domain-specific cleaning as needed
                                return text.strip()
        ...
        if not response_questions:
            ...
            logger.error(f"Context snippet: {context[:300]}")
            return jsonify({'error': 'No questions could be generated from the provided context.'}), 400
